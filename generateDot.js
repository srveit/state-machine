...
const util = require('util'),
  generateDot = ({name, states, inputOptions}) => {
    const options = Object.assign(
      ...
      camelCase = string => (string || '').split(/[- ]/).map(capitalize).join(''),
      objectToString = object =>
      Object.entries(object)
        .map(([value, key]) => `${key}=${value}`).join(', '),
        ...
        if (event.actions) {
          event.actions.map(action => {
            const str = Array.isArray(action) ?
              `${action[0]}(${action.slice(1).map(param => util.inspect(param)).join(', ')})` : action;
      ...
      transitions = states => states.reduce(
        (trans, state) => trans.concat(
          Object.entries(state.events)
            .map(([name, event]) => transitionLine(state.name, event, name))
        ),
        []
      );
